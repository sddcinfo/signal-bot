...
class MessagingService:
    ...
    def __init__(self, db: DatabaseManager, signal_cli_path: str = "/usr/local/bin/signal-cli",
        ...
        # Counter for periodic user consolidation
    ...
    def poll_and_process_messages(self, timeout_seconds: int = 15) -> int:
        ...
        try:
            ...
            # Periodically consolidate duplicate users (every 100 poll cycles)
            self._poll_cycle_count += 1
            if self._poll_cycle_count % 100 == 0:
                try:
                    duplicates_removed = self.db.consolidate_duplicate_users()
                    if duplicates_removed > 0:
                        self.logger.info("Consolidated %d duplicate user entries during polling cycle %d",
                                       duplicates_removed, self._poll_cycle_count)
                except Exception as e:
                    self.logger.warning("Failed to consolidate duplicate users during polling: %s", e)
    ...
    def sync_group_memberships(self) -> bool:
        ...
        try:
            ...
            try:
                duplicates_removed = self.db.consolidate_duplicate_users()
                if duplicates_removed > 0:
                    self.logger.info("Consolidated %d duplicate user entries during sync", duplicates_removed)
            except Exception as e:
                self.logger.warning("Failed to consolidate duplicate users after sync: %s", e)
