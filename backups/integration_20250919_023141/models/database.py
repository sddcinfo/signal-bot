...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            try:
                cursor.execute("ALTER TABLE users ADD COLUMN contact_name TEXT")
            ...
            try:
                cursor.execute("ALTER TABLE users ADD COLUMN profile_given_name TEXT")
            except sqlite3.OperationalError:
                pass

            try:
                cursor.execute("ALTER TABLE users ADD COLUMN profile_family_name TEXT")
            except sqlite3.OperationalError:
                pass
            ...
            # Migration: Add reacted column to messages table if it doesn't exist
            cursor.execute("PRAGMA table_info(messages)")
            columns = [col[1] for col in cursor.fetchall()]
            if 'reacted' not in columns:
                self.logger.info("Adding reacted column to messages table")
                cursor.execute("ALTER TABLE messages ADD COLUMN reacted BOOLEAN DEFAULT FALSE")

                # Migrate existing data: mark messages as reacted if they exist in processed_messages
                cursor.execute("""
                    UPDATE messages
                    SET reacted = TRUE
                    WHERE EXISTS (
                        SELECT 1 FROM processed_messages pm
                        WHERE pm.timestamp = messages.timestamp
                        AND pm.group_id = messages.group_id
                        AND pm.sender_uuid = messages.sender_uuid
                    )
                """)
                migrated_count = cursor.rowcount
                self.logger.info(f"Migrated {migrated_count} messages to reacted=TRUE")

                # Create index on reacted column for performance
                cursor.execute("CREATE INDEX IF NOT EXISTS idx_messages_reacted ON messages(reacted)")

            # Migration: Add sticker metadata columns to attachments table if they don't exist
            cursor.execute("PRAGMA table_info(attachments)")
            attachment_columns = [col[1] for col in cursor.fetchall()]
            if 'pack_id' not in attachment_columns:
                self.logger.info("Adding sticker metadata columns to attachments table")
                cursor.execute("ALTER TABLE attachments ADD COLUMN pack_id TEXT")
                cursor.execute("ALTER TABLE attachments ADD COLUMN sticker_id TEXT")
                self.logger.info("Added pack_id and sticker_id columns to attachments table")
        ...
        try:
            with self._get_connection() as conn:
                ...
                for phone_row in duplicate_phones:
                    ...
                    cursor.execute("""
                        SELECT uuid, friendly_name, created_at, last_seen,
                    ...
                    cursor.execute("""
                        UPDATE users
                        SET friendly_name = ?, message_count = ?, is_configured = ?
                        WHERE uuid = ?
                    """, (merged_friendly_name, merged_message_count, merged_is_configured, canonical_uuid))
        ...
        try:
            ...
            with self._get_connection() as conn:
                ...
                for recipient in recipients:
                    ...
                    # Determine best friendly name
                    friendly_name = None
                    if recipient['given_name']:
                        friendly_name = f"{recipient['given_name']} {recipient['family_name'] or ''}".strip()
                    elif recipient['profile_given_name']:
                        friendly_name = f"{recipient['profile_given_name']} {recipient['profile_family_name'] or ''}".strip()

                    # Update our users table
                    cursor.execute("""
                        INSERT OR REPLACE INTO users (uuid, phone_number, friendly_name, first_seen)
                        VALUES (?, ?, ?, datetime('now'))
                    """, (uuid, phone, friendly_name))
