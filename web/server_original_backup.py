...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def _get_user_timezone(self, query: Dict[str, Any]) -> Optional[str]:
        """Extract user timezone from query parameters with fallback to a default."""
        timezone = query.get('timezone', [None])[0]
        if timezone:
            return timezone
        # Default to Asia/Tokyo if no timezone provided
        return 'Asia/Tokyo'

    def _get_standard_date_selector(self, input_id: str = "date-input", label_text: str = "Select Date:",
                                   button_text: str = "Load Data", onclick_function: str = "loadData()",
                                   auto_load: bool = True, default_today: bool = True,
                                   include_button: bool = True, onchange_function: str = None) -> str:
        """Generate a standardized date selector with consistent styling and behavior."""
        default_value = 'new Date()' if default_today else 'null'

        # Build input with onchange if specified
        onchange_attr = f'onchange="{onchange_function}"' if onchange_function else ''

        # Build button if needed
        button_html = f'<button onclick="{onclick_function}" style="margin-left: 10px; padding: 8px 16px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">{button_text}</button>' if include_button else ''

        auto_load_script = f"""
        // Set default date to today
        document.getElementById('{input_id}').valueAsDate = {default_value};

        // Auto-load data on page load
        window.onload = function() {{
            {onclick_function}
        }};
        """ if auto_load and include_button else f"""
        // Set default date to today
        document.getElementById('{input_id}').valueAsDate = {default_value};
        """
        ...
        <div class="date-selector">
            ...
            <input type="date" id="{input_id}" value="" {onchange_attr} style="margin-left: 10px; padding: 8px; border-radius: 4px; border: 1px solid #ddd;">
            {button_html}
        </div>
        <script>
        {auto_load_script}
    ...
    def do_GET(self):
        ...
        try:
            if path == '/':
                ...
                self._serve_unified_messages(query)
            elif path == '/all-messages':
                # Redirect to new unified messages page
                self._send_redirect('/messages?tab=all')
                return
            elif path == '/messages/by-sender':
                # Redirect to new unified messages page
                group_id = query.get('group_id', [None])[0]
                redirect_url = f'/messages?tab=groups&group_id={group_id}' if group_id else '/messages?tab=groups'
                ...
                if group_id and sender_uuid:
                    self._send_redirect(f'/messages?tab=all&group_id={group_id}&sender_uuid={sender_uuid}')
                else:
                    self._send_redirect('/messages?tab=all')
                return
            elif path == '/sentiment':
    ...
    def _send_redirect(self, location: str):
        ...
        try:
            ...
            self.logger.error("Error sending redirect: %s", e)
        ...
        return """
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                @media (max-width: 768px) {
                    ...
                    .nav { gap: 6px; margin-bottom: 10px; }
                    .nav-item { text-align: center; padding: 8px 12px; font-size: 0.85em; }
                ...
                /* Button-style tabs (used by Users page and Messages page) */
                .tab-btn {
                    padding: 12px 24px;
                    border: none;
                    background: #f8f9fa;
    ...
    def _render_page(self, title, subtitle, content, active_page='', extra_css='', extra_js=''):
        """Generate a complete standardized page with consistent structure."""
        return f"""
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            {self._get_page_header(title, subtitle, active_page)}
    ...
    def _serve_dashboard(self):
        ...
        html = self._render_page('📊 Signal Bot Dashboard', 'UUID-based Signal reaction bot management', content, 'overview')
    ...
    def _serve_setup(self):
        ...
        <body>
            {self._get_page_header('⚙️ Signal Bot Setup', 'Configure and initialize your Signal bot', 'setup')}
    ...
    def _serve_groups(self):
        ...
        for group in groups:
            ...
            <tr>
                ...
                <td>
                    ...
                    {f'<a href="/messages?tab=all&group_id={quote(group.group_id)}" class="btn btn-secondary">View Messages</a>' if group.is_monitored else ''}
                </td>
            </tr>
            """
        ...
        html = self._render_page('👥 Groups Management', 'Configure which groups the bot should monitor', content, 'groups', '', js_code)
        self._send_html_response(html)
                    ...
                    <div id="discovered-tab" class="tab-content">
                        ...
                        <div style="margin: 20px 0;">
                            <span style="font-size: 14px; color: #666;">
                                Duplicate users are automatically consolidated during sync operations.
                            </span>
                        </div>
    ...
    def _serve_messages(self, query: Dict[str, Any]):
        ...
        <body>
            {self._get_page_header(f'💬 Messages from {group.group_name or "Unnamed Group"}', f'Group messages with pagination and filtering', 'all-messages')}

                <div class="card">
                    <div class="nav" style="justify-content: flex-start; gap: 10px;">
    ...
    def _serve_all_messages(self, query: Dict[str, Any]):
        ...
        # Get messages using new server-side filtering
        user_timezone = self._get_user_timezone(query)
        try:
            ...
            {self._get_page_header('💬 All Messages', 'View all messages from monitored groups', 'all-messages')}
                ...
                <div class="card">
                    ...
                    function buildUrl() {{
                        ...
                        if (currentDate) {{
                            params.push('start_date=' + encodeURIComponent(currentDate));
                            params.push('end_date=' + encodeURIComponent(currentDate));
                        }}
    ...
    def _serve_messages_by_sender(self, query: Dict[str, Any]):
        ...
        <body>
            {self._get_page_header(f'👤 Messages by Sender - {group.group_name or "Unnamed Group"}', f'View messages grouped by individual senders', 'all-messages')}
    ...
    def _serve_sender_messages(self, query: Dict[str, Any]):
        ...
        <body>
            {self._get_page_header(f'👤 Messages from {sender_name}', f'Individual messages from a specific sender', 'all-messages')}
    ...
    def _serve_unified_messages(self, query: Dict[str, Any]):
        """Serve unified messages page with tabs for all message functionality."""
        ...
        if tab == 'groups':
            ...
            content = self._render_all_messages_tab(query)
        else:
            # Default to groups
            content = self._render_groups_tab(query)
        ...
        <html>
        <head>
            <title>Messages - Signal Bot</title>
            <style>
                {self._get_standard_css()}
            </style>
        </head>
        <body>
            {self._get_page_header('💬 Messages', 'View and manage all messages from monitored groups', 'messages')}

            <div class="card">
                <div class="tabs">
                    <a href="/messages?tab=groups" class="tab-btn {'active' if tab == 'groups' else ''}">By Group</a>
                    <a href="/messages?tab=senders" class="tab-btn {'active' if tab == 'senders' else ''}">By Sender</a>
                    <a href="/messages?tab=all" class="tab-btn {'active' if tab == 'all' else ''}">All Messages</a>
                </div>
    ...
    def _render_all_messages_tab(self, query: Dict[str, Any]):
        """Render the All Messages tab content (reusing existing _serve_all_messages logic)."""
        page = int(query.get('page', [1])[0])
        per_page = 50
        offset = (page - 1) * per_page
        ...
        if not messages:
            ...
            for msg in messages:
                ...
                timestamp_display = ''
                if timestamp_ms:
                    timestamp_display = f'<span class="timestamp" data-timestamp="{timestamp_ms}"></span>'
                ...
                if attachments:
                    ...
                    for attachment in attachments:
                        ...
                        if file_path and content_type and content_type.startswith('image/'):
                            attachments_html += f'<img src="/attachment/{file_path}" alt="{file_name}" class="attachment-image">'
                        else:
                ...
                <div class="message-item">
                    <div class="message-header">
                        <div class="message-sender">
                            <strong>{msg['sender_display']}</strong>
                            <span class="group-indicator">in</span>
                            <strong class="group-name">{msg['group_display']}</strong>
                        </div>
                        {timestamp_display}
                    </div>
        ...
        if group_filter:
            try:
                ...
                for member in members:
                    ...
                    member_name = member.friendly_name or member.display_name or member.phone_number or member.uuid
                    member_options += f'<option value="{member.uuid}" {selected}>{member_name}</option>'
            except Exception as e:
                self.logger.error("Error getting group members: %s", e)
        ...
        <div class="tab-content">
            ...
            <div class="filters" style="display: flex; gap: 15px; margin: 20px 0; flex-wrap: wrap; align-items: center;">
                <div>
                    <label for="group-filter"><strong>Filter by Group:</strong></label>
                    <select id="group-filter" onchange="filterByGroup(this.value)" style="margin-left: 8px; padding: 5px;">
                        {group_options}
                    </select>
                </div>
                <div>
                    <label for="sender-filter"><strong>Filter by Member:</strong></label>
                    <select id="sender-filter" onchange="filterByMember(this.value)" style="margin-left: 8px; padding: 5px;">
                        {member_options}
                    </select>
                </div>
                <div>
                    <label for="date-filter"><strong>Filter by Date:</strong></label>
                    <input type="date" id="date-filter" onchange="filterByDate(this.value)" style="margin-left: 8px; padding: 4px;">
                </div>
                <div>
                    <label for="attachments-filter" style="display: flex; align-items: center; gap: 8px;">
                        <input type="checkbox" id="attachments-filter" onchange="filterByAttachments(this.checked)" {"checked" if attachments_only else ""} style="margin: 0;">
            ...
            function filterByGroup(groupId) {{
                currentGroupId = groupId;
                currentSenderUuid = ''; // Reset sender when group changes
                window.location.href = buildUrl();
            }}
    ...
    def _render_groups_tab(self, query: Dict[str, Any]):
        ...
        for group in monitored_groups:
            try:
                ...
                message_count = self.db.get_message_count_by_group(group.group_id)
                ...
                <div class="group-card" style="border: 1px solid #ddd; padding: 15px; margin: 10px 0; border-radius: 8px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <a href="/messages?tab=all&group_id={quote(group.group_id)}" class="btn btn-primary">View Messages</a>
                        </div>
                    </div>
                </div>
                """
            except Exception as e:
                self.logger.error("Error getting group stats: %s", e)
                continue
    ...
    def _render_senders_tab(self, query: Dict[str, Any]):
        """Render the By Sender tab content."""
        # Get top senders across all monitored groups
        try:
            # This would need a new database method to get cross-group sender stats
            # For now, show a simple message
            return """
            <div class="tab-content">
                <h3>Messages by Sender</h3>
                <p>Cross-group sender analysis coming soon. For now, use "By Group" tab to view senders within specific groups.</p>
                <p>You can also use the "All Messages" tab and filter by specific senders.</p>
            </div>
            """
        except Exception as e:
            self.logger.error("Error getting sender stats: %s", e)
    ...
    def _api_sync_groups(self):
        ...
        # Automatically consolidate duplicate users after sync
        duplicates_removed = 0
        try:
            duplicates_removed = self.db.consolidate_duplicate_users()
            if duplicates_removed > 0:
                self.logger.info("Consolidated %d duplicate user entries during web sync", duplicates_removed)
        except Exception as e:
            self.logger.warning("Failed to consolidate duplicate users during web sync: %s", e)
    ...
    def _api_users(self):
        ...
        def user_to_dict(user):
            ...
            # Check if user is in any monitored groups
            monitored_groups = self.db.get_user_monitored_groups(user.uuid)
            return {
                'uuid': user.uuid,
                ...
                'in_monitored_groups': len(monitored_groups) > 0,
                'monitored_group_count': len(monitored_groups)
            }
...
<body>
    {self._get_page_header('📊 Activity Visualization', 'Hourly message activity patterns by group', 'activity')}
        ...
        <div class="card">
            {self._get_standard_date_selector("date-input", "Select Date:", "Load Activity", "loadActivityData()", True, True)}
...
<script>
    ...
    def _serve_bot_status(self, query):
        """Serve the bot status page showing uptime history."""
        try:
            # Get bot status history (last 24 hours)
            status_history = self.db.get_bot_status_history(hours=24)
            current_status = self.db.get_current_bot_status()

            html = f"""
            <!DOCTYPE html>
            <html>
            <head>
                <title>Bot Status - Signal Bot</title>
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <style>
                    {self._get_standard_css()}
                    .status-value {{
            ...
            <body>
                <div class="container">
                    {self._get_page_header('🤖 Bot Status', 'View bot uptime and health history', 'bot-status')}

                    <div class="card">
                        <h2>Current Status</h2>
                        {self._format_current_status(current_status)}
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="table-container">
                            <table class="history-table">
                                ...
                                <tbody>
                                    {self._format_status_history(status_history)}
                                </tbody>
                            </table>
                        </div>
                    </div>
            ...
            self.logger.error(f"Error serving bot status page: {e}")
            self._send_error(500, str(e))

    def _format_current_status(self, status):
        """Format current bot status for display."""
        if not status:
            return '<p style="color: #dc3545;">No bot instance running</p>'
        ...
        <div class="status-grid">
            ...
            <div class="status-value">{status['pid']}</div>

            <div class="status-label">Status:</div>
            <div class="status-value {'status-running' if status['status'] == 'running' else 'status-stopped'}">{status['status'].upper()}</div>

            <div class="status-label">Started:</div>
            <div class="status-value">{status.get('started_at', 'Unknown')}</div>

            <div class="status-label">Runtime:</div>
            <div class="status-value">{runtime_str}</div>

            <div class="status-label">Last Heartbeat:</div>
            <div class="status-value">{status.get('last_heartbeat', 'Unknown')}</div>
        </div>
        """

    def _format_status_history(self, history):
        """Format status history for table display."""
        if not history:
            return '<tr><td colspan="7" style="text-align: center; color: #666;">No history available</td></tr>'
        ...
        for record in history:
            try:
                if record['started_at'] and record['last_heartbeat']:
                    started = datetime.fromisoformat(record['started_at'])
                    last_hb = datetime.fromisoformat(record['last_heartbeat'])
                    runtime = last_hb - started
                    hours = int(runtime.total_seconds() // 3600)
                    minutes = int((runtime.total_seconds() % 3600) // 60)
                    runtime_str = f"{hours}h {minutes}m"
                else:
            ...
            status_color = "#28a745" if record['status'] == 'running' else "#dc3545" if record['status'] == 'stopped' else "#ffc107"

            rows.append(f"""
            <tr>
                <td>{record['id']}</td>
                <td>{record['pid']}</td>
                <td style="color: {status_color}; font-weight: bold;">{record['status'].upper()}</td>
                <td>{record.get('started_at', 'N/A')}</td>
                <td>{record.get('last_heartbeat', 'N/A')}</td>
                <td>{runtime_str}</td>
